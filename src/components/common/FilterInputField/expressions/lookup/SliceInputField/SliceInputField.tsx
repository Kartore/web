...
export const SliceInputField: FC<SliceInputFieldProps> = ({
	...
	return (
		...
		>
			...
			<div className={'flex flex-row px-0.5 py-0.5'}>slice from</div>
			{isExpression(from) ? (
				<ExpressionInputField value={from} onChange={onChange} />
			) : (
				<div className={'flex flex-row px-0.5 py-0.5'}>{from.toString()}</div>
			)}
			{to ? (
				<>
					<div className={'flex flex-row px-0.5 py-0.5'}>to</div>
					{isExpression(to) ? (
						<ExpressionInputField value={to} onChange={onChange} />
					) : (
						<div className={'flex flex-row px-0.5 py-0.5'}>{to.toString()}</div>
					)}
				</>
			) : null}
			{children}

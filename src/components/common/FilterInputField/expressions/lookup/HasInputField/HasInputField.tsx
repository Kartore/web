...
export const HasInputField: FC<HasInputFieldProps> = ({
  ...
  return (
    ...
    >
      <div className={'flex flex-row py-0.5 px-0.5'}>has</div>
      {isExpression(propertyKey) ? (
        <ExpressionInputField value={propertyKey} onChange={onChange} />
      ) : (
        <div className={'flex flex-row py-0.5 px-0.5'}>{propertyKey.toString()}</div>
      )}
      ...
      {items ? (
        isExpression(items) ? (
          ...
          <div className={'flex flex-row py-0.5 px-0.5'}>{JSON.stringify(items)}</div>
        )
      ) : (
        <div className={'flex flex-row py-0.5 px-0.5'}>this features</div>
      )}
      {children}
    </div>
  );

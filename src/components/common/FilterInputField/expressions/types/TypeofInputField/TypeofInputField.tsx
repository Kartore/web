...
export const TypeofInputField: FC<TypeofInputFieldProps> = ({
	className,
	children,
	value,
	onChange,
	...
	return (
		...
		>
			<div className={'flex flex-row px-0.5 py-0.5'}>typeof</div>
			{isExpression(value[1]) ? (
				<ExpressionInputField value={value[1]} onChange={onChange} />
			) : (
				<div className={'flex flex-row px-0.5 py-0.5'}>{JSON.stringify(value[1])}</div>
			)}
			{children}
		</div>
	);

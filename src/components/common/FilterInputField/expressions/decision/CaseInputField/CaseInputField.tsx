...
export const CaseInputField: FC<CaseInputFieldProps> = ({
  ...
  return (
    ...
    >
      <div className={'flex flex-row py-0.5 px-0.5'}>CASE</div>
      {expressions.map((expression, index) => {
        ...
        return (
          <Fragment key={'case' + isCondition ? 'condition' : 'value' + index + expression}>
            ...
            {isCondition && <div className={'flex flex-row py-0.5 px-0.5'}>WHEN</div>}
          </Fragment>
        );
      })}
      <div className={'flex flex-row py-0.5 px-0.5'}>ELSE</div>

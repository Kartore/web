...
export const MatchInputField: FC<MatchInputFieldProps> = ({
  ...
  return (
    ...
    >
      <div className={'flex flex-row py-0.5 px-0.5'}>switch</div>
      {expressions.map((expression, index) => {
        const isCondition = index % 2 === 0;
        return (
          <Fragment key={'match' + (isCondition ? 'case' : 'value') + index + expression}>
            {isCondition && <div className={'flex flex-row py-0.5 px-0.5'}>CASE</div>}
            ...
            {isCondition && <div className={'flex flex-row py-0.5 px-0.5'}>:</div>}
          </Fragment>
        );
      })}
      <div className={'flex flex-row py-0.5 px-0.5'}>DEFAULT</div>

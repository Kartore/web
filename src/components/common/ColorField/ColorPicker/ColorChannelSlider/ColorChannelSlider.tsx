...
export const ColorChannelSlider: FC<ColorChannelSliderProps> = ({ ...props }) => {
  ...
  return (
    <div
      {...trackProps}
      ref={trackRef}
      className={cn('h-3 w-full rounded-full border border-gray-300', trackProps.className)}
      style={{
        ...trackProps.style,
    ...
    >
      ...
      >
        <input {...inputProps} ref={inputRef} className={cn('', inputProps.className)} />
      </div>
    </div>
  );
};

...
export const BackgroundLayerPropertiesPanel: FC<
	BackgroundLayerPropertiesPanelProps
> = ({ children, layer, sprite, onChange, className, ...props }) => {
			...
			<div className={'flex flex-col gap-2 px-4'}>
				...
				layer.paint?.['background-color'] === undefined ? (
					<ColorField
						label={'Color'}
						value={parseColor(
							layer.paint?.['background-color'] ?? 'rgba(255, 255, 255, 1)',
						)}
						onChange={(color) => {
							onChange?.(
								layer,
					...
					<TextField
						...
						value={
							layer.paint?.['background-color']
								? String(layer.paint?.['background-color'])
								: '1'
						}
					/>
				)}
				...
				layer.paint?.['background-pattern'] === undefined ? (
					<ComboBox
						...
						onSelectionChange={(value) => {
							if (!value || value === layer.paint?.['background-pattern'])
								return;
